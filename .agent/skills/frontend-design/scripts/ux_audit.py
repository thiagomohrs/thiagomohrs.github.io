...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        if nav_items > 7 and not any(x in filename.lower() for x in ['index', 'pt', 'es']):
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'bootstrap', 'animate', 'colors'}]
            for file in files:
                if '.min.' in file or file.endswith('.bak') or file in {'main.css', 'default.css', 'responsive.css', 'montserrat-fonts.css'}:
                    continue
